...
class UIElement:
    ...
    def display(self):
        ...
        self.draw_bar(screen, 295, 295, self.x+self.health_bar_x_offset, self.y+self.health_bar_y_offset, self.health_bar_width, self.health_bar_height, self.health_colors)
        self.draw_text(screen, self.character.name, 28, self.x+self.name_x_offset, self.y+self.name_y_offset)
        self.draw_text(screen, f"L{self.character.level}", 16, self.x+self.level_x_offset, self.y+self.level_y_offset)
        for i in range(4):
            current_cooldown = self.character.cooldowns[i]
            try:
                max_cooldown = self.character.moves[i].cooldown
                move_name = self.characters.moves.name
            except IndexError:
                max_cooldown = 1
                move_name = ""
            ...
            self.draw_text(screen, move_name, 16, self.x+self.cooldown_bar_x_offsets[i]+self.move_name_x_offset, self.y+self.cooldown_bar_y_offsets[i]+self.move_name_y_offset)
